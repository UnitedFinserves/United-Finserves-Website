        ...
        <div className="mb-24">
          ...
          <div className="relative overflow-hidden">
            <div className="flex gap-6 animate-[marquee_20s_linear_infinite]" style={{ width: 'max-content' }}>
              {[...BANKS, ...BANKS].map((b, i) => (
                <div key={i}
                  className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-gray-50 border border-gray-100 hover:border-brand-gold/30 hover:shadow-md transition-all duration-300 whitespace-nowrap">
          ...
          <div className="relative">
            <AnimatePresence mode="wait">
              <motion.div key={t.id}
                ...
                className="relative rounded-3xl p-10 shadow-2xl border border-gray-100 overflow-hidden"
                style={{ background: 'linear-gradient(135deg, #050D20 0%, #0B2D5E 100%)' }}>

                <div className="absolute inset-0 bg-[radial-gradient(rgba(240,165,0,0.08)_0%,transparent_70%)]" />
                <FaQuoteLeft className="text-brand-gold/20 text-7xl absolute top-6 right-8" />
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-4 border-t border-white/10 pt-6">
                    <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${t.color} flex items-center justify-center text-white font-extrabold text-lg shadow-lg flex-shrink-0`}>
                      {t.initials}
                    </div>
                    <div>
                      <div className="text-white font-extrabold text-base">{t.name}</div>
                      <div className="text-gray-400 text-sm">{t.role}</div>
                      <div className="mt-1.5 inline-block px-3 py-1 rounded-full bg-brand-gold/10 border border-brand-gold/30 text-brand-gold text-xs font-bold">
                        {t.loan}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-2 ml-2">
                {testimonials.map((_, i) => (
                  <button key={i} onClick={() => setActive(i)}
                    className={`rounded-full transition-all ${i === active ? 'w-6 h-2.5 bg-brand-gold' : 'w-2.5 h-2.5 bg-gray-300'}`} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {testimonials.map((item, i) => (
              <motion.div key={item.id}
                ...
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${item.color} flex items-center justify-center text-white font-extrabold text-sm flex-shrink-0`}>
                  {item.initials}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <div className="flex gap-0.5">
                      {[...Array(item.rating)].map((_, si) => <FaStar key={si} className="text-brand-gold text-xs" />)}
                    </div>
                  </div>
